...
| 到底玩了多久 | 住宿的时间算「😴 休息」不算「🎯 游玩」——在酒店睡的 10 小时不该让人以为那天玩了 18 小时。其他地点（比如机场候机）也能手动标成休息 |
| 跨时区那天几点落地 | 时间轴始终按当天的基准时区（出发地）走，跨时区的地点**额外标一行当地时刻**：`🌐 Manila当地 +2 天 05:25`。营业时间也改按当地时间和当地星期几判断，新增一天会继承落脚地的时区 |
| 坐飞机的那段 | 路上时间可以自己填（按路网估算飞行没有意义），填了会标「手填」角标 |

顶部还有当天总览：出门时间、游玩总时长、休息总时长、路上总时长、总里程、吃几顿、几点结束。
...
node test/resttz.js        # 住宿不算游玩、跨时区标当地时刻、手填飞行时间
```
