...
把页面里的时钟冻在固定的一天，再拦掉 Open-Meteo 的预报与历史接口，验证：
预报范围内显示天气条与逐小时角标、下午下雨的地点标出高降水、
同一天不会重复请求、换一天会另查、超出 16 天改用往年同期并注明不是预报、
往年同期没有逐小时因此不标角标、服务返回 500 时安静降级不影响行程。

`test/helpers.js` 里的 `stubWeather` 也被其他浏览器测试引用，避免真的出网。
